...
export class LadderTournament extends Tournament {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (info.name) {
        ...
        if(!fs.existsSync(info.file)) {
          //@ts-ignore
          throw new FatalError(`${info.file} does not exist, check if file path provided is correct`);
        }
      }
      else {
        //@ts-ignore
        if(!fs.existsSync(info)) {
          //@ts-ignore
          throw new FatalError(`${info} does not exist, check if file path provided is correct`);
        }
      }
    }
  ...
  public async run(configs?: DeepPartial<Tournament.TournamentConfigs<LadderConfigs>>) {
    ...
    this.tourneyRunner();
    setInterval(() => {
      this.printTournamentInfo()
      if (Math.min(this.matchQueue.length, this.configs.tournamentConfigs.maxConcurrentMatches - this.matches.size) > 0) {
    ...
    switch(this.configs.rankSystem) {
      case RANK_SYSTEM.TRUESKILL:
        ...
        this.resultProcessingQueue.push(
          {result: resInfo, mapAgentIDtoTournamentID: matchRes.mapAgentIDtoTournamentID});
